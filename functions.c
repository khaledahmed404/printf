...
/**
 * print_string - a fun to print a string
 * @types: a list
		...
		int precision, int size)
{
	...
	while (str[_lenggth] != '\0')
		_lenggth++;

	if (precision >= 0 && precision < _lenggth)
		_lenggth = precision;
	...
	if (width > _lenggth)
	{
		if (flags & F_MINUS)
		{
			write(1, &str[0], _lenggth);
			for (i = width - _lenggth; i > 0; i--)
				write(1, " ", 1);
			return (width);
		}
		else
